...
impl Table {
	...
	pub fn insert(&mut self, mut state: State, player: i32, depth: i32, alpha: i32, beta: i32, score: i32) {
		if player == 1 {
			state.swap();
		}
		...
		let entry = TableEntry{
			...
			depth: depth,
			quality: 
				if score <= alpha { Quality::Upperbound } // le score de `state` est de au maximum `score`
				else if beta <= score { Quality::Lowerbound } // le score de `state` est de au moins `score`
				else { Quality::Exact }
		};
