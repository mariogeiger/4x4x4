...
impl glium::vertex::Vertex for Vertex {
    ...
    fn build_bindings() -> glium::vertex::VertexFormat {
        ...
        Cow::Owned(vec![(Cow::Borrowed("position"), 0, glium::vertex::AttributeType::F32F32F32),
                        (Cow::Borrowed("normal"), 0, glium::vertex::AttributeType::F32F32F32)])
    }
}
...
impl Sphere {
        ...
        where F: glium::backend::Facade
    {
        ...
        positions.push(Vertex { position: (0.0, 0.0, 1.0) });
        ...
        for i in 1..stacks {
            ...
            for j in 0..slices {
                ...
                positions.push(Vertex { position: (x, y, z) });
            }
        }

        positions.push(Vertex { position: (0.0, 0.0, -1.0) });

        let mut indices: Vec<u16> = Vec::with_capacity((3 * slices * (2 + 2 * (stacks - 2))) as
                                                       usize);
